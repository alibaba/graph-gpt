...
# References:
# https://github.com/meta-pytorch/attention-gym/issues/77
# --------------------------------------------------------
def get_flex_dropout_mod(p: float, training: bool = True, device="cuda"):
    """
    结合核心开发者思路的改进版。
    ...
    if p <= 0 or not training:
        return None  # flex_attention 处理 score_mod=None 为 identity

    # 预计算阈值
    threshold = int((1.0 - p) * 0x7FFFFFFF)
    # 随机种子：每一轮训练可以更新这个 seed 来改变 Dropout 模式
    # PyTorch 的 Autograd 系统会自动保存 score_mod 依赖的所有闭包变量（即这个 seed）
    seed = torch.randint(0, 0x7FFFFFFF, (), device=device, dtype=torch.int32)
    # 提前定义好负无穷，避免在内核循环中创建张量
    NEG_INF = float("-inf")

    def dropout_score_mod(score, b, h, q_idx, kv_idx):
        """
        直接利用索引计算一个伪随机数
        基于开发者捕获闭包变量的特性，seed 会被编译进内核
        """
        # 1. 深度混合输入特征
