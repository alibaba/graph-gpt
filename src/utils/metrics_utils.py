...
class SingleLabelClassificationMetrics:
    def __init__(self, device, num_labels=2, **kwargs):
        self.device = device
        if num_labels == 2:
            self.auroc_metric = BinaryAUROC(device=device)
            self.acc_metric = BinaryAccuracy(device=device)
            self.auroc = None
            self.acc = None
            self.all_pred = None
            ...
            raise NotImplementedError(
                f"Metrics for single_label_classification with num_labels={num_labels} is not implemented yet!"
            )
    ...
    def compute(self):
        self.auroc = self.auroc_metric.compute().item()
...
class MultiLabelClassificationMetrics:
    def __init__(self, device, num_labels=2, **kwargs):
        ...
        self.auroc_metric = BinaryAUROC(num_tasks=num_labels, device=device)
        self.auroc_vec = None
