...
dataset_source="ogbl-citation2"

# i. data config
pack_tokens=0
tokenizer_class="StackedGSTTokenizer"  # StackedGSTTokenizer  GSTTokenizer
tokenization_config_file="edge_lvl/ogbl_citation2_tokenization_config.json"
sampling_conf="d1n14"
# CAUTION: in tokenization_config.json, set `sampling`->`edge_ego`->`method`->`name` to `global`

num_cpus=12  # -> consume too much cpu memory
batch_size=256
workerCount=8
total_tokens=2e10  # 1e11  1e9
warmup_tokens=1e9  # 1e9  1e8
max_position_embeddings=1024
samples_per_saving=1000000

ds_prefix="ogbl_citation2"
mid_dir="202409/"

# ii. model config
model_name="medium"  # tiny mini small medium base base24 base48 base64 large xlarge xxlarge
# bf16 for medium, and bf16+small lr for base
stack_method="short"
...
deepspeed_config="./examples/ds_config2_pt_bf16.json"  # in case of minimum-loss-scale-error of model size medium/base

## iv. optimization objective
task_type="pretrain-mlm"  # pretrain  pretrain-mlm  pretrain-ltp  pretrain-euler

suffix="_bf16_${hidden_act}_nmlm_mrlinear_mtp0.8_0_0.2_lr${lr}_adp${attention_dropout}_pdp${path_dropout}_edp${embed_dropout}_mdp${mlp_dropout}_lsi${layer_scale_init_val}_${stack_method}_${stacked_feat_agg_method}_wd${weight_decay}_norm${max_grad_norm}"
